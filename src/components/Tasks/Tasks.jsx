  ...
  const deleteTask = async (id) => {
    try {
      ...
      const newTasks = tasks.map((task) =>
        task.id === id ? { ...task, show: false } : task
      );
      setTasks(newTasks);
      ...
      <main className="container my-5">
        <div className="main-container">
          <div className="container">
            ...
            <ul className="task task-container to-do">
              {tasks.map(
                (task) =>
                  task.state === "to-do" && (
                    <Task
          ...
          <div className="container">
            ...
            <ul className="task task-container in-progress">
              {tasks.map(
                (task) =>
          ...
          <div className="container">
            ...
            <ul className="task task-container done">
              {tasks.map(
                (task) =>
                  task.state === "done" && (
                    <Task
...
const NewTask = ({
  ...
  const [
    { titleTask, responsibility, deadlineTask },
    handleInputChange,
    handleInputReset,
    handleInputEdit,
  ] = useForm({
  ...
  const createTask = async (e) => {
    ...
    if (!responsibility.trim())
      return toast.error("📑 Responsibility level is mandatory");
    ...
    if (edicion) {
      ...
      try {
        await updateDoc(doc(db, user.uid, edicion.id), {
          ...
          tasks.map((task) =>
            task.id === edicion.id
              ? {
                  ...
                  title: titleTask.trim(),
                  responsibility: responsibility.trim(),
                  deadline: calculateDeadLineTask(
                    task.created,
                    deadlineTask.trim()
                  ),
                }
      ...
      try {
        ...
        setTasks((tasks) => [...tasks, { ...newTask, id: data.id }]);
